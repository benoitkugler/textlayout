...
func (met *type2CharstringHandler) Run(op ps.PsOperator, state *ps.Inter) (int32, error) {
	if !op.IsEscaped {
		switch op.Operator {
		case 21: // rmoveto
			...
			if state.ArgStack.Top < 2 {
				return 0, errors.New("invalid rmoveto operator")
			}
