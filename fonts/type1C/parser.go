...
func (p *cffParser) parseTopDict() (*topDictData, error) {
	...
	psi.topDict.underlinePosition = -100
	psi.topDict.underlineThickness = 50
...
var psOperators = [...][2][]psOperator{
	...
	psContextTopDict: {{
		// 1-byte operators.
		0: {+1, "version", func(p *psInterpreter) error {
			p.topDict.version = uint16(p.argStack.a[p.argStack.top-1])
			return nil
		}},
		1: {+1, "Notice", func(p *psInterpreter) error {
			p.topDict.notice = uint16(p.argStack.a[p.argStack.top-1])
			return nil
		}},
		2: {+1, "FullName", func(p *psInterpreter) error {
			p.topDict.fullName = uint16(p.argStack.a[p.argStack.top-1])
			return nil
		}},
		3: {+1, "FamilyName", func(p *psInterpreter) error {
			p.topDict.familyName = uint16(p.argStack.a[p.argStack.top-1])
			return nil
		}},
		4: {+1, "Weight", func(p *psInterpreter) error {
			p.topDict.weight = uint16(p.argStack.a[p.argStack.top-1])
			return nil
		}},
		5:  {-1, "FontBBox", nil},
		13: {+1, "UniqueID", nil},
		...
		0: {+1, "Copyright", nil},
		1: {+1, "isFixedPitch", func(p *psInterpreter) error {
			p.topDict.isFixedPitch = p.argStack.a[p.argStack.top-1] == 1
			return nil
		}},
		2: {+1, "ItalicAngle", func(p *psInterpreter) error {
			p.topDict.italicAngle = math.Float32frombits(uint32(p.argStack.a[p.argStack.top-1]))
			return nil
		}},
		3: {+1, "UnderlinePosition", func(p *psInterpreter) error {
			p.topDict.underlinePosition = math.Float32frombits(uint32(p.argStack.a[p.argStack.top-1]))
			return nil
		}},
		4: {+1, "UnderlineThickness", func(p *psInterpreter) error {
			p.topDict.underlineThickness = math.Float32frombits(uint32(p.argStack.a[p.argStack.top-1]))
			return nil
		}},
		5:  {+1, "PaintType", nil},
		...
		38: {+1, "FontName", func(p *psInterpreter) error {
			p.topDict.cidFontName = uint16(p.argStack.a[p.argStack.top-1])
			return nil
		}},
	}},
