...
func outputDottedCircle(buffer *internal.Buffer) {
	dottedcircle := buffer.OutputGlyph(0x25CC)
	dottedcircle.ResetContinutation()
}

func outputWithDottedCircle(buffer *internal.Buffer) {
	outputDottedCircle(buffer)
	buffer.NextGlyph()
}

func preprocessTextVowelConstraints(buffer *internal.Buffer) {
	if (buffer.Flags & internal.DoNotInsertDottedCircle) != 0 {
		return
	}
