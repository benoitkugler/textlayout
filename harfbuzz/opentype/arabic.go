...
func isWord(genCat cm.GeneralCategory) bool {
	const mask = 1<<cm.Unassigned |
		1<<cm.PrivateUse |
		/*1 << cm.LowercaseLetter |*/
		1<<cm.ModifierLetter |
		1<<cm.OtherLetter |
		/*1 << cm.TitlecaseLetter |*/
		/*1 << cm.UppercaseLetter |*/
		1<<cm.SpacingMark |
		1<<cm.EnclosingMark |
		1<<cm.NonSpacingMark |
		1<<cm.DecimalNumber |
...
func getJoiningType(u rune, genCat cm.GeneralCategory) uint8 {
	...
	const mask = 1<<cm.NonSpacingMark | 1<<cm.EnclosingMark | 1<<cm.Format
	if 1<<genCat&mask != 0 {
		return joiningTypeT
	}
	return joiningTypeU
...
type arabic_shape_plan_t struct {
	...
	* mask_array[NONE] == 0. */
	mask_array [len(arabic_features) + 1]cm.Mask
...
func (cs *complexShaperArabic) dataCreate(plan *hb_ot_shape_plan_t) {
	var arabic_plan arabic_shape_plan_t

	arabic_plan.do_fallback = plan.props.script == language.Arabic
	arabic_plan.has_stch = plan.map_.get_1_mask(newTag('s', 't', 'c', 'h')) != 0
	for i, arabFeat := range arabic_features {
		arabic_plan.mask_array[i] = plan.map_.get_1_mask(arabFeat)
...
func arabicJoining(buffer *cm.Buffer) {
	info := buffer.Info
	prev, state := -1, uint16(0)

	// check pre-context
	for _, u := range buffer.Context[0] {
		thisType := getJoiningType(u, cm.Uni.GeneralCategory(u))
	...
	for i := 0; i < len(info); i++ {
		...
		if entry.prev_action != NONE && prev != -1 {
			info[prev].Aux = entry.prev_action
			buffer.UnsafeToBreak(prev, i+1)
		}

		info[i].Aux = entry.curr_action
...
func mongolianVariationSelectors(buffer *cm.Buffer) {
...
func (cs *complexShaperArabic) setupMasks(plan *hb_ot_shape_plan_t, buffer *cm.Buffer, _ *cm.Font) {
	arabic_plan := cs.plan
	script := plan.props.script
	...
	info := buffer.Info
	for i := range info {
		info[i].Mask |= arabic_plan.mask_array[info[i].Aux]
	}
}

func arabicFallbackShape(plan *hb_ot_shape_plan_t, font *cm.Font, buffer *cm.Buffer) {
...
func recordStch(plan *hb_ot_shape_plan_t, _ *cm.Font, buffer *cm.Buffer) {
	...
	* anything multiplying into 5 pieces, so it's safe-ish... */
	...
	for i := range info {
		if info[i].Multiplied() {
			...
			buffer.Flags |= flagArabicHasStch
		}
	}
}
	...
	* Second pass applies the stretch, copying things to the end of buffer. */
	...
	for step := MEASURE; step <= CUT; step++ {
		...
		for i := originCount; i != 0; i-- {
			...
			/* Yay, justification! */
			var (
				wTotal     cm.Position // Total to be filled
				wFixed     cm.Position // Sum of fixed tiles
				wRepeating cm.Position // Sum of repeating tiles
				nFixed     = 0
				nRepeating = 0
			)
			end := i
			...
			start := i
			context := i
			for context != 0 && !inRange(info[context-1].Aux) &&
			...
			var extraRepeatOverlap cm.Position
			shortfall := sign*wRemaining - sign*wRepeating*(cm.Position(nCopies)+1)
			if shortfall > 0 && nRepeating > 0 {
				nCopies++
				excess := (cm.Position(nCopies)+1)*sign*wRepeating - sign*wRemaining
				if excess > 0 {
					extraRepeatOverlap = excess / cm.Position(nCopies*nRepeating)
				}
			}
...
func arabicFallbackSynthesizeLookupSingle(font *cm.Font, featureIndex int) *lookupGSUB {
...
func arabicFallbackSynthesizeLookupLigature(font *cm.Font) *lookupGSUB {
	var (
...
func arabicFallbackSynthesizeLookup(font *cm.Font, featureIndex int) *lookupGSUB {
	if featureIndex < 4 {
...
type arabic_fallback_plan_t struct {
	...
	mask_array   [ARABIC_FALLBACK_MAX_LOOKUPS]cm.Mask
...
func (fbPlan *arabic_fallback_plan_t) initWin1256(plan *hb_ot_shape_plan_t, font *cm.Font) bool {
...
func newArabicFallbackPlan(plan *hb_ot_shape_plan_t, font *cm.Font) *arabic_fallback_plan_t {
...
func (fbPlan *arabic_fallback_plan_t) shape(font *cm.Font, buffer *cm.Buffer) {
