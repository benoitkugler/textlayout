...
type khmerShapePlan struct {
	...
	mask_array   [KHMER_NUM_FEATURES]cm.Mask
}

//    bool get_virama_glyph (font *cm.Font, rune *pglyph) const
//    {
...
func (cs *complexShaperKhmer) setupMasks(_ *hb_ot_shape_plan_t, buffer *cm.Buffer, _ *cm.Font) {
...
func setKhmerProperties(info *cm.GlyphInfo) {
	u := info.Codepoint
...
func setupSyllablesKhmer(_ *hb_ot_shape_plan_t, font *cm.Font, buffer *cm.Buffer) {
...
func foundSyllableKhmer(syllableType uint8, ts, te int, info []cm.GlyphInfo, syllableSerial *uint8) {
	for i := ts; i < te; i++ {
...
func (khmerPlan *khmerShapePlan) reorderConsonantSyllable(buffer *cm.Buffer, start, end int) {
	info := buffer.Info
...
func (cs *complexShaperKhmer) reorderSyllableKhmer(buffer *cm.Buffer, start, end int) {
...
func (cs *complexShaperKhmer) reorderKhmer(_ *hb_ot_shape_plan_t, font *cm.Font, buffer *cm.Buffer) {
	if cm.DebugMode {
...
func (complexShaperKhmer) preprocessText(*hb_ot_shape_plan_t, *cm.Buffer, *cm.Font) {}
func (complexShaperKhmer) postprocessGlyphs(*hb_ot_shape_plan_t, *cm.Buffer, *cm.Font) {
}
func (complexShaperKhmer) reorderMarks(*hb_ot_shape_plan_t, *cm.Buffer, int, int) {}
