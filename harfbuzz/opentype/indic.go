...
func isOneOf(info *cm.GlyphInfo, flags uint) bool {
...
func isJoiner(info *cm.GlyphInfo) bool {
	return isOneOf(info, JOINER_FLAGS)
}

func isConsonant(info *cm.GlyphInfo) bool {
	return isOneOf(info, CONSONANT_FLAGS)
}

func isHalant(info *cm.GlyphInfo) bool {
...
func setIndicProperties(info *cm.GlyphInfo) {
	u := info.Codepoint
...
func (ws indicWouldSubstituteFeature) wouldSubstitute(glyphs []fonts.GlyphIndex, face cm.Face) bool {
...
type indicShapePlan struct {
	...
	mask_array [INDIC_NUM_FEATURES]cm.Mask
}

func (isp *indicShapePlan) loadViramaGlyph(font *cm.Font) fonts.GlyphIndex {
...
func (indicPlan *indicShapePlan) consonantPositionFromFace(consonant, virama fonts.GlyphIndex, face cm.Face) uint8 {
...
func (cs *complexShaperIndic) setupMasksIndic(plan *hb_ot_shape_plan_t, buffer *cm.Buffer, _ *cm.Font) {
...
func setupSyllablesIndic(_ *hb_ot_shape_plan_t, _ *cm.Font, buffer *cm.Buffer) {
...
func foundSyllableIndic(syllableType uint8, ts, te int, info []cm.GlyphInfo, syllableSerial *uint8) {
	for i := ts; i < te; i++ {
...
func (indicPlan *indicShapePlan) updateConsonantPositionsIndic(font *cm.Font, buffer *cm.Buffer) {
...
func (indicPlan *indicShapePlan) initialReorderingConsonantSyllable(face cm.Face, buffer *cm.Buffer, start, end int) {
	info := buffer.Info
	...
	* We don't need to do this: the normalize() routine already did this for us.
	...
	for i := start; i < base; i++ {
		info[i].ComplexAux = cm.Min8(POS_PRE_C, info[i].ComplexAux)
	}
...
func (indicPlan *indicShapePlan) initialReorderingStandaloneCluster(buffer *cm.Buffer, start, end int) {
...
func (indicPlan *indicShapePlan) initialReorderingSyllableIndic(buffer *cm.Buffer, start, end int) {
...
func (cs *complexShaperIndic) initialReorderingIndic(_ *hb_ot_shape_plan_t, font *cm.Font, buffer *cm.Buffer) {
	if cm.DebugMode {
...
func (indicPlan *indicShapePlan) finalReorderingSyllableIndic(plan *hb_ot_shape_plan_t, buffer *cm.Buffer, start, end int) {
	info := buffer.Info
		...
		if start < newPos && info[newPos].ComplexAux != POS_PRE_M {
			...
			for i := newPos; i > start; i-- {
				if info[i-1].ComplexAux == POS_PRE_M {
					...
					* Indic matra reordering is special and tricky... */
					buffer.MergeClusters(newPos, cm.Min(end, base+1))
			...
			for i := start; i < base; i++ {
				if info[i].ComplexAux == POS_PRE_M {
					buffer.MergeClusters(i, cm.Min(end, base+1))
					break
				}
			}
		}
	...
	*     the following rules:
	...
	if info[start].ComplexAux == POS_PRE_M {
		const flagRange = 1<<(cm.Format+1) - 1<<cm.NonSpacingMark
...
func (indicPlan *indicShapePlan) finalReorderingIndic(plan *hb_ot_shape_plan_t, font *cm.Font, buffer *cm.Buffer) {
...
func (complexShaperIndic) preprocessText(_ *hb_ot_shape_plan_t, buffer *cm.Buffer, _ *cm.Font) {
