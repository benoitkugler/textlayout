...
func generateVowelConstraints(scriptsClasses map[string][]rune, constraintsRunes [][]rune, w io.Writer) {
	...
	fmt.Fprintln(w, "func outputDottedCircle (buffer *common.Buffer) {")
	fmt.Fprintln(w, "	dottedcircle := buffer.OutputGlyph (0x25CC);")
	fmt.Fprintln(w, " 	dottedcircle.ResetContinutation()")
	fmt.Fprintln(w, "}")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "func outputWithDottedCircle (buffer *common.Buffer) {")
	fmt.Fprintln(w, " 	outputDottedCircle (buffer);")
	fmt.Fprintln(w, " 	buffer.NextGlyph ();")
	fmt.Fprintln(w, "}")
	fmt.Fprintln(w)

	fmt.Fprintln(w, `func preprocessTextVowelConstraints (buffer *common.Buffer) {
		if (buffer.Flags & common.HB_BUFFER_FLAG_DO_NOT_INSERT_DOTTED_CIRCLE) != 0 { return }
